...
def get_variable_from_equation(s):
    dict = {8304: '0', 185: '1', 178: '2', 179: '3', 8308: '4', 8309: '5', 8310: '6', 8311: '7', 8312: '8', 8313: '9'}
    # Remove spaces
    s = re.sub(r'\s', '', s)
    # Replace exponents by ^ format
    s = re.sub(r'([⁰¹²³⁴⁵⁶⁷⁸⁹]+)', lambda x:  f'''^{''.join([dict[ord(i)] for i in x.group()])}''', s)
    ...
    data = [tuple(i) for i in map(lambda x: map(lambda x: int(x), x), [i.groups() for i in factorexponent])]
